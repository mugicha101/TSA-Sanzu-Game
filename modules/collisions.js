...
import {ShadowBox, ShadowCircle} from './shadows.js';
import {Player} from './player.js';
export {Collision, HitBox, HitCircle};

class Collision {
static objDict = {}; // stores all collision objects
static nextId = 0;
...
constructor(pos, template = false) {
    this.pos = pos;
    this.enabled = true;
    if (!template) {
        this.id = Collision.nextId;
        Collision.nextId++;
        Collision.objDict[this.id] = this;
    }
}
...
unindex() {
    if (this.shadowObj != null) {
        this.shadowObj.unindex();
    }
    delete Collision.objDict[this.id];
}
reindex() {
    if (this.shadowObj != null) {
        this.shadowObj.reindex();
    }
    Collision.objDict[this.id] = this;
}
...
static isTouching(cA, cB) { // checks if collisions are intersecting each other
    if (!cA.enabled || !cB.enabled) return { overlap: false };
...
dim = [100, 100];
constructor(pos, dim, solid = false, template = false) {
    super(pos, template)
    ...
    this.shadowObj = null;
    if (!template && solid) {
        this.shadowObj = new ShadowBox(pos[0], pos[1], dim[0], dim[1])
        this.shadowObj.pos = this.pos;
        this.shadowObj.dim = this.dim;
    }
}
...
#radius = 10;
constructor(pos, radius, solid = false, template = false) {
    super(pos, template)
    // pos represents topleft corner
    if (!template && solid) {
        this.shadowObj = new ShadowCircle(pos[0], pos[1], radius)
        this.shadowObj.pos = this.pos;
    }
    this.radius = radius;
    this.type = "circle";
...
set radius(radius) {
    this.#radius = radius;
    if (this.solid) this.shadowObj.radius = radius;
}
get radius() {
    return this.#radius;
}
